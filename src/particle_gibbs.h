#ifndef PARTICLE_GIBBS_H
#define PARTICLE_GIBBS_H
...
class ParticleGibbs{
    ...
    public:
        ParticleGibbs(const MatrixXd& Y_, 
                      const int& n_parts_,
...
class NormalMean: public ParticleGibbs{

    public:
        NormalMean(const MatrixXd& Y_, 
                    const int& n_part_, 
                    const long double& rho_upper_,
                    const bool& use_distance_,
                    const MatrixXd& Z_, 
                    ...
                    const VectorXd& Y_pred_) : ParticleGibbs(Y_, n_part_, rho_upper_, use_distance_, Z_, Y_pred_){
            means = means_;
...
class Laplace: public ParticleGibbs{
    public:
        Laplace(const MatrixXd& Y_, 
                const int& n_part_, 
                const long double& rho_upper_, 
                const bool& use_distance_,
                const MatrixXd& Z_,
...
class ARProcess: public ParticleGibbs{

    public:

        ARProcess(const MatrixXd& Y_, 
                  const int& n_part_, 
                  const long double& rho_upper_, 
                  const bool& use_distance_,
                  const MatrixXd& Z_,
                  ...
                  const VectorXd& lambdas_, 
                  const VectorXd& Y_pred_) : 
                        ParticleGibbs(Y_, n_part_, rho_upper_, use_distance_, Z_, Y_pred_){
...
class StateSpace: public ParticleGibbs{

    public:
        StateSpace(const MatrixXd& Y_, 
                   const int& n_part_, 
                   const long double& rho_upper_, 
                   const bool& use_distance_,
                   const MatrixXd& Z_,
                   const VectorXd& mu_means_, 
                   const VectorXd& mu_variances_, 
                   const VectorXd& sigma2s_, 
                   const VectorXd& alphas_, 
                   const VectorXd& betas_, 
                   const int& K_, 
                   const VectorXd& Y_pred_) : ParticleGibbs(Y_, n_part_, rho_upper_, use_distance_, Z_, Y_pred_){
            
            mu_means = mu_means_;
            mu_variances = mu_variances_;
            alphas = alphas_;
            betas = betas_;
            sigma2s = sigma2s_;
            K = K_;
            std::pair<ArrayXd, ArrayXd> grid = get_grid(K, alphas(0), betas(0));
            tau_grid = grid.first;
            tau_grid_diff = grid.second;
            std::cout << tau_grid.transpose() << std::endl;
        }
        ArrayXd tau_grid, tau_grid_diff, tau_grid_vals;
        VectorXd mu_means, mu_variances, sigma2s, alphas, betas;
        int K;
